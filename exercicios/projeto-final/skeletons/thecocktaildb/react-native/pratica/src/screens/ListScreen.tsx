...
import { fetchList, fetchNamesByCategory } from '../api/cocktailDbApi';
import { DrinkSummary, capitalize } from '../types/drink';
import type { RootStackParamList } from '../../App';
import BottomNav from './BottomNav';

type Props = NativeStackScreenProps<RootStackParamList, 'List'>;

const CATEGORIES = ['shot', 'beer', 'shake'];

function apiCategory(slug: string): string {
  switch (slug) {
    case 'shot':
      return 'Shot';
    case 'beer':
...
export default function ListScreen({ navigation }: Props) {
  const [all, setAll] = useState<DrinkSummary[]>([]);
  ...
  const onSelectCategory = async (category: string | null) => {
    ...
    // chamar fetchNamesByCategory(apiCategory(category)) e setCategoryNames()
    // com o resultado. Quando é null, limpar categoryNames.
  ...
  return (
    <TouchableOpacity
      ...
      style={{
        ...
        borderRadius: 16,
        marginRight: 8,
        backgroundColor: selected ? '#d84315' : '#eee',
      }}
    >
